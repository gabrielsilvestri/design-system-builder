...
// Tira os dois blocos antigos e reinsere os novos num ponto fixo. Substituir no
// lugar era frágil: bastava um bloco mudar de posição pra troca virar no-op e o
// arquivo guardar número velho, que é justamente o defeito que este script existe
// pra não ter.
function tiraBloco(texto, chave) {
  const re = new RegExp(`^${chave}:\\n(?:  .*\\n)*`, "m");
  return texto.replace(re, "");
}
saida = tiraBloco(saida, "contrast_notes");
saida = tiraBloco(saida, "contrast_notes_dark");

const juntos = blocoClaro + "\n\n" + blocoEscuro + "\n";
if (/^audit:/m.test(saida)) {
  saida = saida.replace(/^audit:/m, juntos + "\naudit:");
} else {
  // sem bloco audit: entra no fim do frontmatter
  const fim = saida.indexOf("\n---\n", 4);
  saida = saida.slice(0, fim + 1) + juntos + saida.slice(fim + 1);
}
