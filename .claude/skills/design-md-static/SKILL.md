...
Requires Node 18+. The `claude -p` provider needs the [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) on `PATH`. The `openrouter` provider needs `OPENROUTER_API_KEY` set.
...
Fetch HTML (axios) → collect all CSS via cheerio (`<link>`, inline `<style>`, `style=""`, with preload and `@import` resolved) → regex detection pass emitting `stack.json` (Next.js, Tailwind, Radix, GSAP) and `style-fingerprint.json` (shadcn-neutral, carbon-enterprise, apple-glass, polaris-friendly, marketing-gradient) → turndown to markdown for the type specimen `pageCopy` → template `data/url-extract-prompt.txt` → `claude -p` (or OpenRouter) writes `DESIGN.md`, normalized and linted with one retry on max-turns or missing sections → parse frontmatter into `tokens.json` plus `extraction-log.yaml`, quality score and drift report → render single-file `preview.html`. Optional last phase: invoke `DESIGN_MD_POST_HOOK` if set.

Fingerprints feed prose tone, so the LLM matches the archetype instead of producing generic descriptions.
